...
from .utils import *
from course.models import Subcourse  # Ensure the Subcourse model is imported

from django.db.models import Count, Q  # Add Q to your import statement
...
class Quiz(models.Model):
    ...
    subcourse = models.ForeignKey(Subcourse, on_delete=models.CASCADE, related_name='quizzes')
...
class Sitting(models.Model):
    ...
    correct_questions = models.CharField(
        max_length=1024,
        blank=True,
        verbose_name=_("Correct Questions"),
        validators=[validate_comma_separated_integer_list],
    )
    ...
    def get_correct_count(self):
        return len(self.get_correct_questions)

    def get_incorrect_count(self):
        return len(self.get_incorrect_questions)
    ...
    def get_correct_questions(self):
        """Return a list of correct question IDs."""
        if self.correct_questions:
            return list(map(int, self.correct_questions.split(',')))
        return []


    def move_to_previous_question(self):
        question_ids = self._question_ids()  # Get the list of question IDs
        current_question_id = int(self.question_list.split(",")[0])  # Get the current question ID
        
        # Find the index of the current question in the question order
        current_index = question_ids.index(current_question_id) if current_question_id in question_ids else -1
        
        # Determine the previous question index
        if current_index > 0:
            previous_question_id = question_ids[current_index - 1]
            self.question_list = ",".join([str(previous_question_id)] + self.question_list.split(",")[1:])  # Update question_list
            self.save()
            return previous_question_id
        else:
            return None  # No previous question
    ...
    def move_to_next_question(self):
        ...
        if current_index + 1 < len(question_ids):
            ...
            self.question_list = ",".join([str(next_question_id)] + self.question_list.split(",")[1:])  # Update question_list
            self.save()
    ...
    def has_more_questions(self):
        ...
        """
        unanswered_ids = json.loads(self.user_answers).keys()  # IDs of answered questions
        question_ids = self._question_ids()  # All question IDs
        # Determine if there are unanswered questions
        return any(str(q_id) not in unanswered_ids for q_id in question_ids)
    ...
    def get_incorrect_questions(self):
        """Return a list of incorrect question IDs."""
        if self.incorrect_questions:
            return list(map(int, self.incorrect_questions.split(',')))
        return []
    ...
    def result_message(self):
        if self.check_if_passed:
            return _(f"You have passed this quiz, congratulations")
        else:
    ...
    def get_questions(self, with_answers=False):
        ...
        if with_answers:
            ...
            for question in questions:
                question.user_answer = user_answers.get(str(question.id), None)
    ...
    def get_current_question(self):
        """
        Returns the current question for this sitting based on the order of questions
        and the user's progress.
        """
        if not self.question_list:
            return None  # or handle this case as appropriate

        question_ids = [int(q_id) for q_id in self.question_list.split(",") if q_id]
        answered_ids = json.loads(self.user_answers).keys()
        unanswered_ids = [q_id for q_id in question_ids if str(q_id) not in answered_ids]

        if not unanswered_ids:
            return None  # No more questions left to answer

        # Get the first unanswered question ID
        current_question_id = unanswered_ids[0]
        return Question.objects.get_subclass(id=current_question_id)
...
class MCQuestion(Question):
    ...
    # Assuming guesses is a list of selected choice IDs
         correct_choices = Choice.objects.filter(question=self, correct=True).values_list('id', flat=True)
    
    # Check if all guesses are in the correct choices
         return all(guess in correct_choices for guess in guesses) and len(guesses) == len(correct_choices)
...
class Note(models.Model):
    ...
    def __str__(self):
        return f"Note by {self.user} on {self.quiz} - {self.question}"
