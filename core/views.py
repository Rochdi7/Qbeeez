...
def dashboard_view(request):
    ...
    # Get programs count
    program_count = Program.objects.count()  # Adjust as necessary for your model
    ...
    context = {
        ...
        "program_count": program_count,  # Add programs count to the context
...
def post_add(request):
    if request.method == "POST":
        form = NewsAndEventsForm(request.POST, request.FILES)  # Include request.FILES for file uploads
...
def support_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            university_phone = form.cleaned_data['university_phone']
            ...
            send_mail(
                subject,
                f"Message from {name} ({email}, {university}, {university_phone}):\n\n{message}",
                settings.DEFAULT_FROM_EMAIL,
                [settings.SUPPORT_EMAIL],  # Email address to receive support messages
                fail_silently=False,
            )
